...
% Copyright (c) 2022-2023 Cheng He
    ...
    while Global.NotTermination(Population)
        ...
        for g = 1 : Par.N
            DV = find(Par.Dec(g,:)==1);
            Population = SecondOptimization(Population,DV,1);
            drawnow();
            PV = find(Par.Dec(g,:)==0);
...
function [Population,fitness] = SparseFit(Archieve,Par,mask)
    ...
    for i = 1 : Par.N
        ...
        fitness(i) = min(calCon(Range,TEMP.objs));
...
function [Par,TEMP] = ReformulatedOptimization(Population,Par)
    [TEMP,Par.fit]       = SparseFit(Population,Par,Par.Dec>0);
        for i = 1 : Par.gen
            MatingPool  = TournamentSelection(2,2*Par.N,Par.fit);    
